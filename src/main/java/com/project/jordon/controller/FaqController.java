...
import com.project.jordon.service.FaqService;
...
@RequestMapping("/faq/*") //컨트롤 자체에 매핑주소
public class FaqController {
    ...
    public String board_write_ok(FaqVO f, RedirectAttributes rttr, HttpSession session, MemberVO m) {
        /* BoardVO b는 board_write.jsp의 네임피라미터 이름과 BoardVO 빈클래스의 변수명이 같으면 b객체에 글쓴이와 제목,글내용이 저장되어 있다.
         * 코드라인을 줄이는효과가 발생한다.
         */
        session.getAttribute("session");
        this.faqService.insertBoard(f);//게시물저장
        return "redirect:/faq/board_list";
    }//board_write_ok()


    //게시물 목록
    @GetMapping("/board_list") //get으로 접근하는 매핑주소 board_list등록
    public String board_list(Model m, HttpServletRequest request, FaqVO f, HttpSession session, MemberVO mb) {
        ...
        int listCount = this.faqService.getListCount();
        //총게시물수
        List<FaqVO> blist = this.faqService.getBoardList(f);
        //게시물 목록
        ...
        return "faq/board_list"; //뷰리졸브 경로(뷰페이지 경로) => /WEB-INF/views/board/board_list.jsp
    }//board_list()


    //내용보기+조회수 증가
    @RequestMapping("board_cont") //get or post 방식일때 모두 실행
    public ModelAndView board_cont(@RequestParam("fbno") int fbno, @RequestParam("page") int page, HttpSession session,MemberVO m) {
        String memberid = (String) session.getAttribute("session");
        session.setAttribute("session", memberid);

        FaqVO b = this.faqService.getBoardCont(fbno);//조회수증가와 내용보기
        ...
        cm.setViewName("faq/board_cont2");//뷰페이지 경로
        return cm;
    }//board_cont()

    //게시물 수정폼
    @RequestMapping("/faq/board_edit")
    public String board_edit(int fbno, int page,
                             HttpServletRequest request, MemberVO m, HttpSession session) {
        //int bno,int page로 지정하면 get으로 전달된 bno,page피라미터값을 가져옴
        FaqVO eb = faqService.getBoardCont2(fbno);//조회수를 증가하지 않고 번호에 해당하는 레코드를 가져오기
        request.setAttribute("eb", eb);
        request.setAttribute("page", page);
        String memberid = (String) session.getAttribute("session");
        session.setAttribute("session", memberid);
        return "faq/board_edit";
    }

    //수정완료
    @RequestMapping("board_edit_ok") //get or post 양쪽방식으로 접근하는 board_edit_ok 매핑주소 처리
    public ModelAndView board_edit_ok(int fbno,int page,FaqVO eb,HttpSession session) {

        String memberid = (String) session.getAttribute("session");
        session.setAttribute("session", memberid);
        eb.setFbno(fbno);//번호값을 저장
        this.faqService.updateBoard(eb);//번호를 기준으로 제목,글쓴이,글내용을 수정

        ModelAndView em=new ModelAndView();
        em.setViewName("redirect:/faq/board_cont");//매핑주소 경로 설정
        em.addObject("fbno",fbno);
    ...
    public ModelAndView board_del(int fbno, int page,RedirectAttributes rttr, HttpSession session, MemberVO m) {
        String memberid = (String) session.getAttribute("session");
        session.setAttribute("session", memberid);
        faqService.deleteBoard(fbno);//게시물 삭제
        ModelAndView dm=new ModelAndView();
        dm.setViewName("/faq/board_list");
        return dm;
    }//board_del()
}
